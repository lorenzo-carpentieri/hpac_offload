...
public:
   ...
   **/
  inline PixelMatrix toPixelMatrix();
   ...
   **/
  inline void fromPixelMatrix(const PixelMatrix&);
};
...
inline void load_bitmap_mirrored(string filename, int size, std::vector<float4>& input) {
  Bitmap input_image;
  ...
  for(size_t i = 0; i < size; i++)
    for(size_t j = 0; j < size; j++) {
      ...
      float4 color = {static_cast<float>(pixel.r), static_cast<float>(pixel.g), static_cast<float>(pixel.b), 0.f}; // cast to float
...
inline void save_bitmap(string filename, int size, const std::vector<float4>& output) {
  ...
  for(size_t i = 0; i < size; i++) {
    ...
    for(size_t j = 0; j < size; j++) {
      float4 color = {output[i * size + j].x ,output[i * size + j].y, output[i * size + j].z , output[i * size + j].w};
